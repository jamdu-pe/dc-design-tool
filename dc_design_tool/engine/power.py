...
def size_electrical(it_kw: float, cooling_kw: float, rack_count: int,
    ...
    pf = dist["power_factor"]
    house_ratio = dem["house_load_ratio"]
    house_kw = it_kw * house_ratio
    ...
    gen_kw = calc.generator_kw(it_kw, cooling_kw, house_ratio,
                               el["generator"]["start_margin"]) * harm["generator_factor"]
    ...
    mv_demand_kw = facility_kw * (1 + dem["design_margin"])
    ...
    pue_est = calc.pue(it_kw, cooling_kw, it_kw * dem["distribution_loss_ratio"])

    electrical = {
        "facility_kw": round(facility_kw, 1),
        "house_kw": round(house_kw, 1),
        "house_load_ratio": house_ratio,
        "power_factor": pf,
        "distribution_loss_ratio": dem["distribution_loss_ratio"],
        "demand_factor": dem["demand_factor"],
